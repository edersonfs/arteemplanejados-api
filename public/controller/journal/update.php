...
try {        
    ...
    // Take old images
    $oldJournal = $journal->getById($_POST['id']);
    
    // Handle top image upload if provided
    $image_file = $oldJournal['image_file'];
    $image_path = $oldJournal['image_path'];
    ...
    if ($journal->existsByTitleWhenEdit($data['title'], $data['id'])) {
      echo json_encode([
