...
try {
    ...
    $oldCompany = $company->getById($_POST['id']);
    if (!$oldCompany) {
        echo json_encode(array("message" => "record_does_not_exist"));
        exit;
    }

    $image_file = $oldCompany['image_file'];
    $image_path = $oldCompany['image_path'];
    ...
    if (!empty($data['cnpj']) && $company->existsByCnpjWhenEdit($data['cnpj'], $data['id'])) {
        echo json_encode([
            "message" => "cnpj_already_exists"
        ]);
        exit;
    }

    if (!empty($data['email']) && $company->existsByEmailWhenEdit($data['email'], $data['id'])) {
        echo json_encode([
