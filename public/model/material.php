...
class Material
{
  ...
  public function create($data)
  {
    $query = "INSERT INTO `" . $this->table_name . "`
            (company_id, supplier_id, material_type_id, name, description, unit_cost, sale_price, stock,
    ...
    $stmt->bindValue(
      ...
      $data['material_type_id'] === null || $data['material_type_id'] === ''
        ? PDO::PARAM_NULL
        : PDO::PARAM_INT
    );
  ...
  private function selectColumnsSql(): string
  {
    return "
      ...
      mat.material_type_id as material_type_id,
      mty.name as material_type_name,
      mat.name as name,
  ...
  private function fromJoinSql(): string
  {
    return "
            FROM
                ...
                left join `material_type` mty on mat.material_type_id = mty.id
                inner join `user` upus on mat.updated_user_id = upus.id
                inner join `user` crus on mat.created_user_id = crus.id";
  }

  public function getAll($company_id, $material_type)
  {
    $query = "SELECT " . $this->selectColumnsSql() . $this->fromJoinSql() . "
            WHERE (mat.company_id = :company_id)
            AND (LOWER(IFNULL(mty.name, '')) = LOWER(:material_type) or :material_type is null or :material_type = '')
            ORDER BY mat.name";

    $stmt = $this->conn->prepare($query);
    $stmt->execute(['company_id' => $company_id, 'material_type' => $material_type]);
  ...
  public function getPagination($limit, $offset, $company_id, $material_type)
  {
    $query = "SELECT " . $this->selectColumnsSql() . $this->fromJoinSql() . "
            WHERE (mat.company_id = :company_id)
            AND (LOWER(IFNULL(mty.name, '')) = LOWER(:material_type) or :material_type is null or :material_type = '')
    ...
    $query = "
            ...
            WHERE
                ...
                AND (
                    ...
                    OR LOWER(IFNULL(mty.name, '')) LIKE LOWER('%$search%')
