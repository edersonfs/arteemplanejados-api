...
class Supplier
{
  ...
  public function create($data)
  {
    $query = "INSERT INTO `" . $this->table_name . "`
            (company_id, name, contact, phone, email, address, city, state,
  ...
  private function selectColumnsSql(): string
  {
    return "
                sup.id as id,
                sup.company_id as company_id,
                cmp.name as company_name,
                sup.name as name,
                sup.contact as contact,
                sup.phone as phone,
                sup.email as email,
                sup.address as address,
                sup.city as city,
                sup.state as state,
                sup.image_file as image_file,
                sup.image_path as image_path,
                sup.created_user_id as created_user_id,
                sup.created_date as created_date,
                crus.name as created_user_name,
                sup.updated_user_id as updated_user_id,
                sup.updated_date as updated_date,
  ...
  private function fromJoinSql(): string
  {
    return "
            FROM
                `" . $this->table_name . "` sup
                inner join `company` cmp on sup.company_id = cmp.id
                inner join `user` upus on sup.updated_user_id = upus.id
                inner join `user` crus on sup.created_user_id = crus.id";
  }
  ...
  public function search($search, $limit, $offset, $company_id)
  {
    $query = "
            ...
            WHERE
                ...
                AND (
                    LOWER(IFNULL(sup.name, '')) LIKE LOWER('%$search%')
                    OR LOWER(IFNULL(sup.contact, '')) LIKE LOWER('%$search%')
                    OR LOWER(IFNULL(sup.email, '')) LIKE LOWER('%$search%')
                    OR LOWER(IFNULL(sup.phone, '')) LIKE LOWER('%$search%')
                    OR LOWER(IFNULL(sup.address, '')) LIKE LOWER('%$search%')
                    OR LOWER(IFNULL(sup.city, '')) LIKE LOWER('%$search%')
                    OR LOWER(IFNULL(sup.state, '')) LIKE LOWER('%$search%')
